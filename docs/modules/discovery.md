...
1. **读取画像**
   ...
   真正进入发现策略时，画像会被压缩成更容易消费的摘要。比如 `SearchStrategy` 会取前几个高权重兴趣、核心特质、deep needs 和 `disliked_topics` 来生成 query；`ExploreStrategy` 则会额外看探索开放度，判断这轮适不适合往陌生方向走。
...
| v0.3.x disliked_topics 闭环 | ✅ | `build_profile_summary()` 会把 `profile.preferences.disliked_topics[:8]` 带入 discovery profile summary，让 search / trending / explore / YouTube query 生成和 batch 内容评估都能显式看到长期避雷项 |
...
| 策略 | 消费的画像字段 | 用途 |
|------|-------------|------|
| **SearchStrategy** | `personality_portrait`, `core_traits[:5]`, `interests[:10]`, `favorite_up_users[:5]`, `disliked_topics[:8]`, `deep_needs[:5]`, `_active_speculations` | 生成搜索词、计算兴趣锚定分，并避开长期雷点 |
