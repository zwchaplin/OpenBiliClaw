...
一个纯本地、私有、开源的自进化跨平台内容发现 Agent：从你的跨平台使用、反馈和对话中持续深化心理画像，带着对你的理解主动去 B 站、小红书、抖音、YouTube 等来源找内容。
...
- 🚀 **图形化引导初始化** — 不想进命令行也行：插件「推荐」tab 会列出前置清单（B 站登录 / LLM / embedding），点「开始初始化」就能在界面里一步步看着画像与首轮内容池建起来（命令行 `openbiliclaw init` 仍是等价入口）
