...
## 📌 跨平台内容统一进入待评估池（2026-06-04）

- **B 站 / 小红书 / 抖音 / YouTube 评估流程对齐** —— 各来源只负责先找到 raw candidates，统一写入 `discovery_candidates` 待评估池。
- **统一 evaluator 混源 batch 判断喜不喜欢** —— 后端按来源混合取样，结合 Soul 画像、来源上下文和近期负反馈样本统一打分，再决定是否进入推荐池。
- **推荐池上限仍按真实可换数生效** —— `pool_available_count >= pool_target_count` 时停止 discovery / drain，不会因为待评估池存在而继续无限找内容。
